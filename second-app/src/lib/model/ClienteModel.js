...
class ClienteModel  {
  ...
  consultarClienteById(idCliente){
    ...
    return new Promise((resolve, reject) => {    
      ...
      localStorage.getConnection().transaction(tx => {
        tx.executeSql(
          `SELECT idCliente as key,idCliente,clave,nombre_comercial,razon_social,rfc,calle,no_ext,no_int,colonia,Clientes.id_municipio,
                  dom_municipio.nombre as municipio,Clientes.id_estado,dom_estado.nombre as estado,codigo_postal,telefono,celular,
                  nivel_socioeconomico_id,NivelSocioeconomico.nivel_socioeconomico,Clientes.has_credito,Clientes.dias_credito,Clientes.importe_credito,Clientes.saldo_favor,
                  Clientes.id_status
             FROM Clientes
