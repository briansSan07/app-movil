...
class Pagando extends Component {
  ...
  guardarVenta() {
    if (this.state.cambio <= 0 && this.state.fechaSeteada) {
      ...
      .then((result) => {
        ...
        if (result) {
          this.props.navigation.navigate("Pagada", { folio: result.folio, idVenta: result.idVenta });
        } else {
          console.error("Error: Resultado de salvarVenta es undefined.");
        }
      })
      .catch((error) => {
  ...
  render() {
    ...
    return (
      <SafeAreaView style={{flex:1}}>
        ...
        <View style={{backgroundColor: '#51747F', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', bottom:0, flex:0 }}>
          <View style={{flex:1}}>
            <TouchableOpacity
          ...
          <View style={{flex:1}}>
            ...
            >
              <View style ={{position: 'relative'}}>
                <Icon name="add" style={{ color: 'white', fontSize: 25 }} />
              </View>
